...
\end{lstlisting}
        ...
        \textbf{ERROR(\$pag)}: É um método protegido, que recebe como parâmetro o nome da página que não foi encontrada no sistema. Sua funcionalidade é, inicialmente, verificar se a variável \textbf{\$error} é \textbf{TRUE} \emph{(Verdadeiro)} ou \textbf{FALSE} \emph{(False)}, para posteriormente, direcionar o usuário para a página de erro padrão do sistema ou exibir uma mensagem dizendo se o erro foi causado pela falta de um \emph{Controller} ou de uma \emph{Action} para o sistema.

        O método é escrito dessa forma:
...
protected function ERROR($pag) {
    if($this->error) {
        switch ($pag) {
        case 'controller':
            die("O Controller ".$this->_controller." nao foi criado.");
            break;
        case 'action':
            die("A action ".$this->_action." nao foi criada.");
        default:
            break;
        }
    } else {
